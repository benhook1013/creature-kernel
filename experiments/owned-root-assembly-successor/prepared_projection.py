...
PARTS = (("pelvis", None), ("torso", None), ("neck", None), ("upper_arm", "left"), ("upper_arm", "right"),
    ("thigh", "left"), ("thigh", "right"))
STATIONS = (("lower_pelvis", "pelvis", "form_torso_profile_lower_pelvis"),
    ("upper_pelvis", "pelvis", "form_torso_profile_upper_pelvis"),
    ("lower_abdomen", "torso", "form_torso_profile_lower_abdomen"),
    ("waist_abdomen", "torso", "form_torso_profile_waist_abdomen"),
    ("upper_abdomen", "torso", "form_torso_profile_upper_abdomen"),
    ("lower_ribcage", "torso", "form_torso_profile_lower_ribcage"),
    ("upper_ribcage_shoulder", "torso", "form_torso_profile_upper_ribcage_shoulder"),
    ("neck_collar", "neck", "form_head_neck_profile_neck_collar"),
    ("neck_upper", "neck", "form_head_neck_profile_neck_upper"))
SHOULDER_DIMS = (("start_lateral", "form_arm_profile_upper_arm_start_lateral_radius"),
    ("start_up", "form_arm_profile_upper_arm_start_up_radius"),
    ("start_forward", "form_arm_profile_upper_arm_start_forward_radius"),
    ("shoulder_depth", "form_shoulder_depth_radius"))
HIP_DIMS = (("r_x", "form_leg_profile_thigh_start_lateral_radius"), ("r_y", "form_leg_profile_thigh_start_up_radius"),
    ("r_z", "form_leg_profile_thigh_start_forward_radius"))
SHOULDER_SUMS = (("axilla", "form_axilla"), ("peak", "form_shoulder_peak"), ("arm_origin", None))
HIP_SUMS = (("P_s", "form_leg_profile_thigh_start"),)
SIDE_CONFIGS = (("shoulders", "upper_arm", (("axilla", "form_axilla", "form_shoulder_control"),
      ("peak", "form_shoulder_peak", "form_shoulder_control")), SHOULDER_DIMS),
    ("hips", "thigh", (("P_s", "form_leg_profile_thigh_start", "form_leg_profile_control"),), HIP_DIMS))
BODY_COUNTS = {"attachments": 1, "capabilities": 3, "dimensions": 153, "fields": 0,
    "frames": 16, "joints": 17, "landmarks": 43, "modules": 1,
    "parts": 18, "regions": 4, "sockets": 2}
SOURCE_ROW_FIELDS = {"dimensions": {"owner", "role", "value"}, "frames": {"owner", "role", "transform"},
    "landmarks": {"frame", "owner", "position", "role"}}
PROFILE_IDS = ("standard_neutral_reference", "compact_broad_short_limb_large_head",
    "tall_narrow_long_legged", "slender_long_limb", "stocky_broad_chested")
PROFILE_DIMENSION_KEYS = frozenset((  # noqa: SIM905 -- compact frozen key universe
    "arm_profile_forward arm_profile_lateral arm_profile_up arm_radius arm_shoulder body_extent_x "
    "body_extent_y body_extent_z body_profile_depth body_profile_lateral foot_extent_x foot_extent_y "
    "foot_extent_z foot_profile_forward foot_profile_lateral foot_profile_up hand_extent_x hand_extent_y "
    "hand_extent_z head_extent_x head_extent_y head_extent_z head_profile_forward head_profile_lateral "
    "head_profile_up leg_profile_forward leg_profile_lateral leg_profile_up leg_radius neck_profile_forward "
    "neck_profile_lateral neck_profile_up neck_radius tail_root_end tail_root_start tail_tip_end tail_tip_start"
).split())
...
def _public_admission(function):
    @wraps(function)
    def guarded(*args, **kwargs):
        try:
            return function(*args, **kwargs)
        except PreparedProjectionError:
            raise
        except Exception as exc:
            raise PreparedProjectionError(f"{function.__name__}: malformed admission") from exc
    return guarded
def _need(test, where, message):
    if not test:
        raise PreparedProjectionError(f"{where}: {message}")
def _obj(value, where):
    _need(type(value) is dict, where, "expected object")
    return value
def _keys(value, expected, where):
    _obj(value, where)
    valid = all(type(key) is str for key in value) and set(value) == set(expected)
    _need(valid, where, "unknown, missing, or non-string field")
    return value
...
def _attempt(function, args, caught, where, message, kwargs=None):
    try:
        return function(*args, **(kwargs or {}))
    except caught as exc:
        raise PreparedProjectionError(f"{where}: {message}") from exc
def _number(value, where, *, runtime=False, positive=False, wire=False):
    if wire:
        result = _attempt(_COERCE_BINARY64, (value,), artifacts.ArtifactSerializationError,
    ...
    _need(math.isfinite(result) and (not positive or result > 0.0), where, f"expected finite {label}number")
    return result
...
def _decode_source_json(raw, where):
    _need(type(raw) is bytes, where, "expected raw bytes")
    caught = (UnicodeDecodeError, json.JSONDecodeError, RecursionError, TypeError, ValueError)
    text = _attempt(raw.decode, ("utf-8",), caught, where, "source is not strict UTF-8 JSON")
    value = _attempt(json.loads, (text,), caught, where, "source is not strict UTF-8 JSON",
                     {"object_pairs_hook": _pairs, "parse_constant": _constant})
    return _obj(value, where), _SHA256_BYTES(raw)
def _read(path, where): return _attempt(_READ_FILE, (path,), (OSError, TypeError, ValueError), where, "read failed")
def _fixed_path(path, expected, where):
    candidate = _attempt(Path, (path,), (TypeError, ValueError), where, "invalid path").absolute()
...
def normalize_source_address(raw):
    ...
    for key in ("namespace", "kind", "role"):
        _text(value[key], f"source address.{key}")
...
def _address(raw):
    value = normalize_source_address(raw)
...
def _runtime_address(value, where):
    ...
    _text(kind, f"{where}[2]")
    _text(role, f"{where}[3]")
...
def _source_transform(raw, where, *, zero_translation=False):
    transform = _keys(raw, {"translation", "rotation_xyzw"}, where)
    translation = _vector(transform["translation"], f"{where}.translation")
    rotation = _vector(transform["rotation_xyzw"], f"{where}.rotation_xyzw", 4)
    _need(tuple(rotation) == _ROTATION, where, "rotation must be exact identity")
    _need(not zero_translation or translation == [0.0, 0.0, 0.0], where, "frame translation must be zero")
    return translation
def _validate_selector_rows(rows, name):
    selectors, frame_references = {}, set()
    for index, raw in enumerate(rows):
        where = f"body.{name}[{index}]"
        row = _keys(raw, SOURCE_ROW_FIELDS[name], where)
        owner = _address(row["owner"])
        role = _text(row["role"], f"{where}.role")
        _need((owner, role) not in selectors, where, "duplicate selector")
        selectors[owner, role] = row, index
        if name == "dimensions":
            _number(row["value"], f"{where}.value", positive=True)
        elif name == "frames":
            _source_transform(row["transform"], f"{where}.transform", zero_translation=True)
        else:
            frame = _keys(row["frame"], {"owner", "role"}, f"{where}.frame")
            frame_owner = _address(frame["owner"])
            frame_role = _text(frame["role"], f"{where}.frame.role")
            _need(frame_owner == owner, f"{where}.frame", "wrong frame owner")
            frame_references.add((frame_owner, frame_role))
            _vector(row["position"], f"{where}.position")
...
def _validate_source_document(source):
    ...
    _fixed_scalars(source["basis"], _BASIS_ITEMS, "source.basis")
    ...
    _keys(body, BODY_COUNTS, "source.body")
    valid_counts = all(type(body[name]) is list and len(body[name]) == count for name, count in BODY_COUNTS.items())
    _need(valid_counts, "source.body", "wrong required record counts")
    parts = {}
    for index, raw in enumerate(body["parts"]):
        where = f"body.parts[{index}]"
        row = _keys(raw, {"address", "containment", "placement"}, where)
        address = _address(row["address"])
        _need(address not in parts, where, "duplicate part address")
        _source_transform(row["placement"], f"{where}.placement")
        _obj(row["containment"], f"{where}.containment")
        parts[address] = row, index
    dimensions, _ = _validate_selector_rows(body["dimensions"], "dimensions")
    frames, _ = _validate_selector_rows(body["frames"], "frames")
...
def _admit_source_bytes(raw):
    source, digest = _decode_source_json(raw, "source")
    _need(len(raw) == _SIZES[2] and digest == _HASHES[1], "source", "source identity mismatch")
...
def _validate_profile_table(table):
    ...
    _fixed_scalars(table["base_source"], (("document", _SOURCE_DOCUMENT), ("namespace", "main"),
        ("path", _ROLES[1]), ("sha256", _HASHES[1]),
    ), "profile table.base_source")
    profiles = table["profiles"]
    _need(type(profiles) is list and len(profiles) == len(PROFILE_IDS), "profile profiles", "wrong count")
    for index, raw in enumerate(profiles):
        where = f"profile table.profiles[{index}]"
        row = _keys(raw, {"dimension_scales", "id", "label", "part_placements"}, where)
        _literal(row["id"], PROFILE_IDS[index], f"{where}.id")
        _text(row["label"], f"{where}.label")
        scales = _obj(row["dimension_scales"], f"{where}.dimension_scales")
        _keys(scales, PROFILE_DIMENSION_KEYS, f"{where}.dimension_scales")
    ...
    return profiles[0]
def _admit_profile_bytes(raw):
    table, digest = _decode_source_json(raw, "profile table")
    _need(len(raw) == _SIZES[3] and digest == _HASHES[2], "profile table", "profile identity mismatch")
    return _validate_profile_table(table), raw, digest
def _verify_contract(path=_PATHS[0]):
    contract = _fixed_path(path, _PATHS[0], "contract path")
    sidecar = _fixed_path(_PATHS[1], _PATHS[1], "contract sidecar path")
    contract_raw, sidecar_raw = _read(contract, "contract"), _read(sidecar, "contract sidecar")
    _need(len(contract_raw) == _SIZES[0] and _SHA256_BYTES(contract_raw) == _HASHES[0], "contract", "identity mismatch")
    _need(len(sidecar_raw) == _SIZES[1] and sidecar_raw == _SIDECAR_CONTENT, "contract sidecar", "identity mismatch")
def _fixed_inputs(source_path, contract_path, profile_path):
    _verify_contract(contract_path)
    source = _fixed_path(source_path, _PATHS[2], "source path")
    profile = _fixed_path(profile_path, _PATHS[3], "profile table path")
    admission = _admit_source_bytes(_read(source, "source"))
    selected, raw, digest = _admit_profile_bytes(_read(profile, "profile table"))
...
def _world(parts, chain):
    ...
    result = [0.0, 0.0, 0.0]
    for owner in chain:
        placement = parts[owner][0]["placement"]
        result = _add(result, _vector(placement["translation"], "part placement.translation"))
    return result
def _select_position(admission, owner, role, frame_role, worlds):
    ...
    _need(_address(frame["owner"]) == owner and frame["role"] == frame_role,
          f"body.landmarks[{index}].frame", "wrong landmark frame")
    _select(admission["frames"], owner, frame_role, "frames")
    position = _vector(row["position"], f"body.landmarks[{index}].position")
    return _add(worlds[owner], position)
...
def _source_files(source_digest, source_size, profile_digest, profile_size):
    records = ({"path": _ROLES[1], "sha256": source_digest, "bytes": source_size},
               {"path": _ROLES[2], "sha256": profile_digest, "bytes": profile_size})
    return sorted(records, key=lambda record: record["path"].encode("utf-8"))
def _identity(path, digest): return {"path": path, "sha256": digest}
def _walk(value, spec, where, *, wire):
    if spec is str:
        return _text(value, where, nonempty=False)
    if spec is int:
        _need(type(value) is int, where, "expected integer")
        return value
    if spec is _ADDRESS:
        _runtime_address(value, where)
        return value
    ...
    if kind == "binary64":
        return _number(value, where, runtime=not wire, positive=argument and not wire, wire=wire)
    if kind == "vector":
        return _vector(value, where, argument, runtime=not wire, wire=wire)
    ...
    return value
def _validate_prepared_structure(prepared, *, wire=False): return _walk(prepared, _PREPARED_SPEC, "prepared", wire=wire)
...
def _chains():
    ...
    for side in ("left", "right"):
        result[_a("upper_arm", side)] = (pelvis, torso, _a("upper_arm", side))
        result[_a("thigh", side)] = (pelvis, _a("thigh", side))
    return result
def _part_output(parts, owner):
...
def _station_suffixes(role):
    return (("lateral_radius", "forward_radius", "forward_radius") if role == "neck"
            else ("lateral_radius", "anterior_radius", "posterior_radius"))
def _station_output(admission, worlds, role, prefix):
    ...
    radii = [_dimension(admission, owner, f"{prefix}_{suffix}")[0]
             for suffix in _station_suffixes(role)]
    return {"owner": _address_json(owner), "prefix": prefix,
            "C": _select_position(admission, owner, prefix, frame, worlds),
            "rL": radii[0], "rA": radii[1], "rP": radii[2]}
def _side_output(admission, worlds, side, part_role, positions, dimensions):
    owner = _a(part_role, side)
    result = {field: _select_position(admission, owner, role, frame, worlds)
              for field, role, frame in positions}
    if part_role == "upper_arm":
        result["arm_origin"] = list(worlds[owner])
    for field, role in dimensions:
        result[field] = _dimension(admission, owner, role)[0]
    return result
def _build_prepared(admission, profile, profile_raw, profile_digest):
    parts, chains = admission["parts"], _chains()
    worlds = {owner: _world(parts, chain) for owner, chain in chains.items()}
    sides = {
        group: {side: _side_output(admission, worlds, side, role, positions, dimensions)
                for side in ("left", "right")}
        for group, role, positions, dimensions in SIDE_CONFIGS
    }
    prepared = {
        "schema": _PREPARED_SCHEMA,
        "contract": _identity(_ROLES[0], _HASHES[0]),
        "source": _identity(_ROLES[1], _HASHES[1]),
        "profile_selection": {
            "profile_id": _NEUTRAL_PROFILE, "profile_table_path": _ROLES[2],
            "profile_table_sha256": profile_digest,
            "dimensions": {key: float(number) for key, number in profile["dimension_scales"].items()},
        },
        "basis": dict(_BASIS_ITEMS),
        "parts": [_part_output(parts, _a(role, side)) for role, side in PARTS],
        "stations": {n: _station_output(admission, worlds, r, p) for n, r, p in STATIONS},
        "shoulders": sides["shoulders"],
        "hips": sides["hips"],
        "provenance": {"source_files": _source_files(
            admission["sha256"], len(admission["bytes"]), profile_digest, len(profile_raw))},
    }
...
def validate_prepared(prepared):
    ...
    value = _validate_prepared_shape(prepared)
    expected = _build_prepared(*_fixed_inputs(_PATHS[2], _PATHS[0], _PATHS[3]))
    _need(_JSON_BYTES(value) == _JSON_BYTES(expected), "prepared", "values do not match fixed projection")
    return value
@_public_admission
def admit_prepared_bytes(raw):
    """Decode and admit exact canonical prepared bytes with schema-aware floats."""
    _need(type(raw) is bytes, "prepared bytes", "expected bytes")
    decoded = _attempt(_DECODE_JSON, (raw,), (artifacts.ArtifactSerializationError, RecursionError),
                       "prepared bytes", "invalid canonical JSON")
    value = _validate_prepared_structure(decoded, wire=True)
    _need(_JSON_BYTES(value) == raw, "prepared bytes", "coercion changed canonical bytes")
    return validate_prepared(value)
@_public_admission
def prepare_standard_neutral(source_path=_PATHS[2], *, contract_path=_PATHS[0], profile_table_path=_PATHS[3]):
    """Admit fixed inputs and return the exact standard-neutral object."""
    return _build_prepared(*_fixed_inputs(source_path, contract_path, profile_table_path))
...
def _bind(component, derivation, addresses, pointers):
...
def _validate_binding_records(records):
    ...
    _need(canonical_json_sha256(components) == _BINDING_HASHES[0], "source bindings", "component universe mismatch")
    _need(canonical_json_sha256(records) == _BINDING_HASHES[1], "source bindings", "mapping mismatch")
    return records
def _append_sum(bindings, admission, parts, chains, component, owner, landmark_role):
...
@_public_admission
def source_binding_records(source_path=_PATHS[2], *, contract_path=_PATHS[0], profile_table_path=_PATHS[3]):
    admission, _, _, _ = _fixed_inputs(source_path, contract_path, profile_table_path)
    parts, bindings, chains = admission["parts"], [], _chains()
    for name, role, prefix in STATIONS:
        owner = _a(role)
        dimensions = tuple((field, f"{prefix}_{suffix}")
                           for field, suffix in zip(("rL", "rA", "rP"), _station_suffixes(role)))
        _append_group(bindings, admission, parts, chains, f"stations.{name}", owner, (("C", prefix),), dimensions)
    for side in ("left", "right"):
        _append_group(bindings, admission, parts, chains, f"shoulders.{side}",
                      _a("upper_arm", side), SHOULDER_SUMS, SHOULDER_DIMS)
        _append_group(bindings, admission, parts, chains, f"hips.{side}", _a("thigh", side), HIP_SUMS, HIP_DIMS)
    ordered = sorted(bindings, key=lambda item: item["prepared_component"].encode("utf-8"))
    return _validate_binding_records(ordered)
build_source_binding_records = source_binding_records
canonical_json_bytes = _JSON_BYTES
def canonical_json_sha256(value): return _SHA256_BYTES(_JSON_BYTES(value))
__all__ = [
