...
def _ownership_counts(surface, evaluation, chart_api, mesh_api, prepared, formulas, summary, level):
    ...
    boundary = {edge for edge, faces in uses.items() if len(faces) == 1}; records.extend(_edge_candidates(uses, owners, boundary, junction_edges)); records.extend((("boundary", port), "boundary", all(edge in boundary for edge in {tuple(sorted((left, loop[(slot + 1) % len(loop)]))) for slot, left in enumerate(loop)}) and len(boundary) == sum(len({tuple(sorted((left, loop[(slot + 1) % len(loop)]))) for slot, left in enumerate(values)}) for _, values in mesh.boundary_loops)) for port, loop in mesh.boundary_loops)
...
def _geometry(surface, mesh_api, chart, anatomy, prepared):
    ...
    evaluation = surface.evaluate(prepared); meshes = (evaluation.cage, *evaluation.levels); formulas = tuple(surface.formula_candidate_records(prepared)); chart_summary = chart.build_chart_summary(evaluation, formulas); chart.validate_chart_summary(chart_summary, evaluation, formulas); anatomy.validate_evaluated_surface(evaluation, chart_summary); anatomy_rows = tuple(anatomy.anatomy_gate_records(evaluation, prepared, chart_summary))
    ...
    for level, (mesh, report) in enumerate(zip(meshes, reports)):
        ...
        invalid_values = _invalid_counts(mesh, report, edge, triangles, _ownership_counts(surface, evaluation, chart, mesh_api, prepared, formulas, chart_summary, level), surface); [_put(structural, thresholds, _gate(f"structural.L{level}.invalid_count.{name}", 1, invalid_values[name], invalid_values[name], "eq", 0, 0, "count")) for name in invalid]
...
def _causality(surface, render, mesh_api, prepared, geometry):
    ...
    if tuple(surface.MUST_AFFECT_PARAMETER_IDS) != PARAMETER_IDS or type(surface.PERTURBATION_DELTA_M) is not float or surface.PERTURBATION_DELTA_M != 0.01: _fail("must-affect inventory or exact binary64 delta differs from the frozen contract")
    for parameter in PARAMETER_IDS:
        component = surface.MUST_AFFECT_COMPONENTS.get(parameter)
        if component is None: _fail(f"missing prepared selector for {parameter}")
        derivative = surface.propagate_derivative(geometry["evaluation"].cage, surface.analytic_control_derivatives(prepared, component), level=2)
        predicted = tuple(index for index, point in enumerate(derivative) if any(value != 0.0 for value in point))
        observed = surface.evaluate_perturbation(prepared, parameter, levels=2).levels[1]
        ...
        perturbations.append({"parameter_id": parameter, "prepared_component": component, "delta_m": float(surface.PERTURBATION_DELTA_M), "support_level": 2, "predicted_support_count": len(predicted), "observed_support_count": len(actual), "predicted_support_sha256": _support_hash(predicted), "observed_support_sha256": _support_hash(actual), "maximum_movement_m": maximum, "artifact": None})
...
def validate_seed_bundle(*, root, seed, identity):
    ...
    try: prepared = prepared_api.admit_prepared_bytes(prepared_raw)
    ...
    perturbations = causality["perturbations"]; expected_parameters = list(_order(PARAMETER_IDS)); _need(type(perturbations) is list and [item.get("parameter_id") if type(item) is dict else None for item in perturbations] == expected_parameters and tuple(surface.MUST_AFFECT_PARAMETER_IDS) == PARAMETER_IDS and surface.PERTURBATION_DELTA_M == 0.01, "runtime or manifest must-affect inventory differs")
    ...
    for index, row in enumerate(perturbations):
        ...
        _need(value["prepared_component"] == surface.MUST_AFFECT_COMPONENTS.get(parameter) and _binary64(value["delta_m"], f"causality perturbation[{index}].delta_m") == 0.01 and value["support_level"] == 2 and type(value["support_level"]) is int, f"causality perturbation selector, delta, or level differs: {parameter}")
...
def build_seed(output_path):
    ...
    try:
        ...
        mark = time.perf_counter(); geometry = _geometry(surface, mesh_api, chart, anatomy, prepared); geometry["structural"] = catalog + geometry["structural"]; geometry["thresholds"] = catalog_thresholds + geometry["thresholds"]
        ...
        mark = time.perf_counter(); perturbations, perturb_payloads = _causality(surface, render, mesh_api, prepared, geometry); timings.append({"phase": "causality", "seconds": float(time.perf_counter() - mark)})
