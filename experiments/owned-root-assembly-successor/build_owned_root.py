...
def _geometry(surface, mesh_api, chart, anatomy, geometry_input):
    ...
    for level, mesh in enumerate(meshes):
        inputs = _junction_inputs(surface, evaluation, level, chart_summary); report = mesh_api.validate_geometry(mesh.vertices, mesh.quads, level, dict(mesh.boundary_loops), _directions(surface), expected, inputs, mesh.face_owners); reports.append(report); intersection_reports.append(report["intersection_report"])
