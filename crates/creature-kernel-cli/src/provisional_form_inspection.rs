...
const FORMAT: &str = "creature-kernel.provisional-form-preview.v7";
    ...
    match build_provisional_form_preview(source, ResourceProfile::ORDINARY) {
        Ok(preview) => match prepare_single_source(source, ResourceProfile::ORDINARY) {
            Ok(prepared) => match prepare_authored_controls(&prepared) {
                Ok(controls) => {
                    match prepare_authored_dimensions(&preview, &prepared, &controls.torso_profile)
                    {
...
fn is_authored_control_owner(address: &AddressKey) -> bool {
    is_upper_arm_side_owner(address)
        || (address.anchors().is_empty() && matches!(address.role(), "pelvis" | "torso"))
}
...
fn result(value: Value) -> CliResult {
    ...
    CliResult {
        json: creature_kernel_core::provisional_json::to_string(&value).unwrap_or_else(|_| {
            r#"{"format":"creature-kernel.provisional-form-preview.v7","operation":"inspect-provisional-form","status":"internal-failure","stage":"output","diagnostics":[{"code":"ck.cli.provisional-form.output-serialization","message":"could not serialize provisional form inspection result"}]}"#.to_owned()
        }),
        exit_code,
    }
}
...
mod tests {
    ...
    fn biped_success_has_four_named_variants_and_eighteen_descriptors() {
        ...
        assert_eq!(value["authored_dimensions"].as_array().unwrap().len(), 57);
        assert_eq!(value["authored_landmarks"].as_array().unwrap().len(), 11);
        assert_eq!(value["authored_frames"].as_array().unwrap().len(), 4);
        assert_eq!(
    ...
    #[test]
    fn v7_keeps_the_historical_shoulder_vocabulary_closed_and_torso_refs_nonduplicated() {
        let value = parsed(&inspect_source(&example()));
