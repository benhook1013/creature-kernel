# Outcome roadmap

Status: Provisional planning baseline

This roadmap defines outcomes rather than dates. Movement between stages depends
on evidence and accepted decisions.

## Stage 0: Govern the unknowns

Exit outcomes:

- Canonical documentation authority has a Proposed lightweight process and
  remains pending Ben's disposition.
- Consequential decisions have a lightweight decision record and one-pass
  adversarial review when the trigger applies.
- Open research questions and experiments have stable identifiers.
- Documentation and decision-record structure remain operational under the
  provisional trial; consolidated validation reports their current state.

## Stage 1: Prove semantic body generation

Exit outcomes:

- A minimal body document and resolved body graph exist.
- One bounded furry morphology family is defined.
- Several substantially different bodies generate without a handcrafted base mesh.
- Semantic part identity and local fields survive surface generation.
- Builds are reproducible through a headless command.

## Stage 2: Prove embodiment

Exit outcomes:

- Generated skeletons and skinning pass reference poses.
- Collision proxies and semantic contact regions are generated.
- Multiple generated bodies share one animation or control scenario.
- Structured diagnostics identify known geometry and rigging failures.

## Stage 3: Prove bounded real-time interaction

Exit outcomes:

- A runtime avatar package loads through the first host adapter.
- Two generated characters establish semantic contact.
- Local deformation and physical response run within a declared hardware budget.
- Quality tiers and at least one fallback path are demonstrated.

## Stage 4: Prove extensibility

Candidate outcomes:

- A second morphology module or substantial structural variation.
- A first external-mesh conformance path.
- More advanced regional deformation.
- Versioned package compatibility and migration.

## Later productization

Potential outcomes, not commitments:

- Creator-facing GUI.
- Published SDK and engine adapters.
- User and technical-artist workflows.
- Release, security, and artifact-distribution processes.
- Optional cinematic-quality pipeline.
