...
| Historical failed surface-candidate experiment | Historical | retired | The bounded candidate over the five ordered source profiles, including `standard_neutral_reference`, is retained as historical evidence. It closed after the unchanged exact-five expansion failed before rendering or publication; no human checkpoint or Product Stage 1 exit resulted. A new candidate requires a new Ben-authorized runway; this retained evidence does not bypass or activate Readiness 3 or Readiness 4, speculative packages, a final surface representation, or the formal multi-branch comparison below | `experiments/` as historical candidate evidence | Exact ordered-profile and landmark provenance, technical triage, structural checks, and scoped exact-five failure evidence |
| Product Stage 2 generation fixtures | Next | planned | Product Stage 2 only, distinct from the already admitted Readiness 2 parser/bootstrap corpus: a separate Ben-approved readiness/decision record names the reviewed source commit reference, manifest path/digest, SHA-256 payload digest, exact ordered path/mode/content set containing only the manifest and declared schema, fixtures, and snapshots, and the versioned external path-set framing/profile (exact identifier remains readiness-gated); readiness/approval/successor records, mutable pointers, self-reference, and Git commit identity are excluded; rerun on the merged target compares those content identities even when the merge commit changes; append-only successors and unlisted fixtures do not activate independently | `fixtures/body-documents/` | Immutable binding, manifest/listed-source agreement, and deterministic output checks |
| Readiness 4 geometry exploration | Next | planned | Readiness 4 only: a working resolver plus provisional geometry profile and project-owned GeometryRequest/GeometryResult seam; this readiness gate is distinct from product Stage 1 and product Stage 4. CK-KICK-014 is exploratory and does not require accepting/reactivating parked DR-0009/0010 | `experiments/` under an activated proof | Bounded exploratory evidence; generated bundles remain ephemeral/unretained and do not select production geometry |
...
| Host-engine adapter docs | Next | planned | Accepted Readiness 3, the repository adapter trigger through its accepted adapter DR, and DR-0013 T4 disposition are prerequisites; only then does the post-Readiness-3 transaction define signed permutation/positive scale, storage/output-only or runtime-conformance tier, and precision/subnormal probes. Disposable experiment-host evidence may precede these conditions but is not the adapter | Architecture and implementation area | Adapter smoke tests and FTZ/DAZ capability evidence |
